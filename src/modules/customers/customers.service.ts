...
export class CustomersService {
      ...
      include: {
        invoices: {
          ...
          select: {
            ...
            paidAmount: true, status: true, issueDate: true,
          },
        },
      },
    });
  ...
  async getStatement(id: string, companyId: string) {
    ...
    const totalInvoiced = invoices.reduce((sum, inv) => sum.plus(new Decimal(inv.totalAmount)), new Decimal(0));
    const totalPaid = invoices.reduce((sum, inv) => sum.plus(new Decimal(inv.paidAmount)), new Decimal(0));
